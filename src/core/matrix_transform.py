...
def extrinsic_to_modelview(rvec, tvec, R_vector = True):
    ...
    for idx in range(n_rvec):
        R, _ = cv2.Rodrigues(rvec[idx])
        ...
        tvec_new = tvec[0].flatten().reshape((3, 1))

        transform_matrix_list.append(Rx @ np.hstack((R, tvec_new)))
    transform_matrix_mean = np.zeros((3, 4))
    for transform_matrix in transform_matrix_list:
        transform_matrix_mean += transform_matrix
    
    M[:3, :] = transform_matrix_mean / n_rvec
    return M
